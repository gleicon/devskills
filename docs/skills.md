...
- **Args:** an optional range — `vPREV..vNEXT` verbatim (any git range), a single ref (previous tag..ref), or nothing (last two version tags; with no tags it asks). `--record [path]` also writes `RETRO.md` in the current directory or at `path`: newest-first sections, rule bullets before evidence, idempotent per range.
...
Activate [Tiger Style](https://tigerstyle.dev/) engineering constraints for the session: safety, explicitness, bounded everything, assertions, no silent fallbacks. Active during all review skills too.
...
- **Reach for it when:** any build session where you want clean, honest, human-readable history as you go. Stacks with `/ds-tiger-style-mode` + `/ds-test-mode` (disciplined code *and* disciplined history).
...
- **Reach for it when:** before merging non-trivial work, auditing an area you suspect is decaying, or reviewing AI-generated code where multiple agents may have introduced competing implementations. Pairs naturally with `/ds-deslop` (inline slop removal) and `/ds-grill-me` (plan interrogation).
...
- **Args:** treated as scope (files, directories, globs); defaults to docs changed on the current branch. `--full` widens scope to all docs. Inline code comments are out of scope — that's `/ds-comment-review`.
...
- **Args:** treated as scope (files, directories, globs); defaults to tests covering code changed on the current branch. `--full` widens scope to the whole suite.
...
- **Args:** treated as scope (files, directories, globs); defaults to UI code changed on the current branch. `--full` widens scope to the whole UI.
...
Scan the project's dependency manifests and lockfiles for known vulnerabilities via the [OSV](https://osv.dev/) database (CVEs and advisories), using the `osv-scanner` binary. For each hit it reports the package, the vulnerable and fixed versions, the advisory, and whether the vulnerable symbol is actually reachable when call-graph data is available.
...
- **Args:** scope (files, directories, globs); defaults to code changed on the current branch. `--full` widens scope to the whole codebase. `--max-level=<1|2|3>` (or freeform "free wins only") clamps higher tiers; `--no-tiger` skips the Tiger Style section.
