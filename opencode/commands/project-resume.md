...
4. If `.project/handoff.md` exists, check whether it is still current:
   - Compare its modification time against `.project/PLAN.md` and the latest commit (`git log -1 --format=%cd`).
   - If the handoff is newer than both, load it — it's the freshest context.
   - If it is older than either, treat it as **stale**: mention it exists and its date, but do not rely on it.
