...
- With `--fix`: apply only findings whose fix is **mechanical and unambiguous** — narrowing a wildcard to a stated allowlist, adding a blocked-pattern entry, pinning a dependency version, removing a logged secret. Anything that changes what the agent is able to do, alters approval logic, or rests on an assumption you could not verify **stays report-only**: a wrong fix to an agent's permissions is worse than none. Edit only files tracked in the repository under review; an exported or pasted artifact is always report-only, because a fix there never reaches the system it came from. After applying, re-run any check already in the loop and revert anything that breaks or that touched more than intended. Close with what was applied and what was left.
