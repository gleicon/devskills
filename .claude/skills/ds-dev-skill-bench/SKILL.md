...
Before the first bench invocation, echo the resolved targets with each one's scenario count and total run count, and ask for the mode if it is still unresolved. One confirmation for the whole batch, not one per skill. Zero bench runs happen before this gate.
